...
const currencyMoney = (currency: string, amount: number) =>
  `${currency} ${round2(amount).toFixed(2)}`;

const toSafeArray = (charges?: InvoiceCharge[]) => charges ?? [];

const sumCharges = (charges: InvoiceCharge[], payer: InvoiceCharge["payer"]) =>
  round2(charges.filter((charge) => charge.payer === payer).reduce((sum, charge) => sum + charge.amount, 0));
...
export const calculateInvoiceTotals = (invoice: InvoiceTemplateData): Totals => {
  ...
  const total =
    invoice.total ?? round2(subtotal + tax + shipping + chargesTotal - discount);
...
export const generateInvoicePdfBuffer = async (invoice: InvoiceTemplateData): Promise<Buffer> => {
  ...
  if (invoice.shipment?.incoterm) doc.text(`Incoterm: ${invoice.shipment.incoterm}`);
  ...
  if (charges.length) {
    doc.fontSize(10).text(
      `Importer charges: ${currencyMoney(invoice.currency, totals.importerChargesTotal)}`,
      { align: "right" }
    );
    doc.text(
      `Exporter charges: ${currencyMoney(invoice.currency, totals.exporterChargesTotal)}`,
      { align: "right" }
    );
...
export const generateInvoiceExcelBuffer = async (invoice: InvoiceTemplateData): Promise<Buffer> => {
  const totals = calculateInvoiceTotals(invoice);
  ...
  if (invoice.dueDate) sheet.addRow(["Due Date", invoice.dueDate]);
  if (invoice.transportMode) sheet.addRow(["Transport Mode", getTransportLabel(invoice.transportMode)]);
  if (invoice.shipment?.origin && invoice.shipment?.destination) {
    sheet.addRow(["Shipment Route", `${invoice.shipment.origin} -> ${invoice.shipment.destination}`]);
  }
  if (invoice.shipment?.incoterm) sheet.addRow(["Incoterm", invoice.shipment.incoterm]);
  if (invoice.shipment?.transportDocumentNumber) {
  ...
  if (invoice.seller.address) sheet.addRow(["Seller Address", invoice.seller.address]);
  sheet.addRow(["Buyer", invoice.buyer.name]);
  if (invoice.buyer.address) sheet.addRow(["Buyer Address", invoice.buyer.address]);
  if (invoice.buyer.email) sheet.addRow(["Buyer Email", invoice.buyer.email]);
  sheet.addRow([]);
...
export const generateInvoiceImageBuffer = async (
  ...
  const push = (text: string, size = 16, weight = 400) => {
    svgRows.push(
      `<text x="60" y="${y}" font-family="Arial, sans-serif" font-size="${size}" font-weight="${weight}" fill="#0f172a">${escapeXml(
        text
      )}</text>`
    );
    y += size + 12;
  ...
  if (invoice.dueDate) push(`Due: ${invoice.dueDate}`, 16, 400);
  ...
  invoice.items.forEach((item, index) => {
    const amount = round2(item.quantity * item.unitPrice);
    push(
      `${index + 1}. ${item.description} | qty ${item.quantity} x ${currencyMoney(
        invoice.currency,
        item.unitPrice
      )} = ${currencyMoney(invoice.currency, amount)}`,
      14,
      400
    );
  });
  ...
  if (charges.length) {
    ...
    charges.forEach((charge) => {
      push(
        `${charge.code} ${charge.label} (${charge.payer}) ${currencyMoney(
          invoice.currency,
          charge.amount
        )}`,
        14,
  ...
  push(
    `Importer/Exporter/Shared: ${currencyMoney(
      invoice.currency,
      totals.importerChargesTotal
    )} / ${currencyMoney(invoice.currency, totals.exporterChargesTotal)} / ${currencyMoney(
      invoice.currency,
      totals.sharedChargesTotal
    )}`,
    14,
    500
  );
